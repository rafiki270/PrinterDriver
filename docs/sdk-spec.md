...
3. **Printing interface as standard as possible.** The public API models standard receipt
   ...
   drawer). Raster deserves equal billing with text: existing POS renders receipts to canvas
...
4. **Works with the most standard printers.** Support target = the printers already
   deployed/implemented in the existing POS monorepo (see §9) plus the Xprinter XP-S260M
...
Support target: every printer currently implemented/used in the existing POS monorepo
(`the POS monorepo`, checkout at `/Users/dictator/Projects/monorepo`), plus the
...
## 10. Current state in the existing POS monorepo (what the SDK replaces)
...
- **Raster is a first-class job type.** existing POS prints receipts as rendered images, not
...
- **Job payloads stay opaque; routing stays app-side.** The POS distinguishes 15
  ...
  startPreparation, internalMessage, image, card-terminalReceipt) routed to per-role printers
